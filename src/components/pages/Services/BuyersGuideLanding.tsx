...
export default function BuyersGuideLanding(props: MyProps) {
  const { allData } = props;
    ...
    <main>
      ...
      <div className='bg-[#E6E6E6] py-14 md:py-20'>
        <h2 className='px-10 text-center text-2xl md:text-3xl lg:text-4xl'>
          {allData?.pages?.nodes[0]?.buyerGuide?.topBannerTitle}
        </h2>
        <div
          className='md:text-md mx-auto mt-5 max-w-[1400px] px-10 text-center text-xs lg:text-lg'
          dangerouslySetInnerHTML={{
            __html: allData?.pages?.nodes[0]?.buyerGuide?.topBannerDescription,
          }}
        ></div>
      </div>
      <h2 className='px-4 py-5 text-center text-2xl md:px-10 md:py-5 md:text-3xl lg:py-10 lg:text-4xl'>
        {allData?.pages?.nodes[0]?.buyerGuide?.propertySelling?.title}
      </h2>

      {allData?.pages?.nodes[0]?.buyerGuide?.propertySelling?.sellingDiv?.map(
        (item: any, idx: number) => (
          <div key={idx}>
      ...
      <ContactForm
        fields={fields}
        phone={allData?.pages?.nodes[0]?.buyerGuide?.contactSection?.phone}
        email={allData?.pages?.nodes[0]?.buyerGuide?.contactSection?.email}
        address={allData?.pages?.nodes[0]?.buyerGuide?.contactSection?.address}
        heading={false}
      />
