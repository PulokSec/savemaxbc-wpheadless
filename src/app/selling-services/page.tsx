...
export default async function SellingServices() {
  const { data } = await getClient().query({
    query,
    context: {
      fetchOptions: {
    ...
    <>
      <main className='max-w-screen mt-10 bg-[url("https://savemaxheadlessdemo.csoft.ca/wp-content/uploads/2023/10/Middle-part-bg.png")] bg-cover bg-center bg-no-repeat'>
          ...
          <div className='text-center'>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html:
                  data?.pages?.nodes[0]?.sellingServices?.topFeatureDescription,
              }}
            ></div>
          </div>
        </div>
        <AdviceServiceFeature
          featuredData={
            data?.pages?.nodes[0]?.sellingServices?.serviceFeatureSection
              ?.featuredDiv
          }
        />
        <BottomServiceSection
          bottomSection={data?.pages?.nodes[0]?.sellingServices?.bottomSection}
        />
        <Footer
